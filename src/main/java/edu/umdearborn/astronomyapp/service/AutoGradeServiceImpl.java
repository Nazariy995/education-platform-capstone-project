...
public class AutoGradeServiceImpl implements AutoGradeService {
	...
	public List<Question> getGatekeepers(String moduleId, int pageNum) {
		return entityManager
				.createQuery("select q from Question q join q.page p join p.module m where m.id = :moduleId and "
						...
						+ "p.order < :pageNum and m.id = :moduleId group by p.order)", Question.class)
	...
	public boolean checkAnswer(String answerId) {
		...
		if (QuestionType.NUMERIC.equals(answer.getQuestion().getQuestionType())) {
			return checkNumeric(answer, entityManager.find(NumericQuestion.class, answer.getQuestion().getId()));
	...
	protected boolean checkNumeric(Answer answer, NumericQuestion question) {
		...
		BigDecimal upperLimit = question.getCorrectCoefficient().add(question.getAllowedCoefficientSpread())
				.movePointRight(question.getCorrectExponenet());
		BigDecimal lowerLimit = question.getCorrectCoefficient().subtract(question.getAllowedCoefficientSpread())
				.movePointRight(question.getCorrectExponenet());
	...
	protected boolean checkMultipleChoice(Answer answer, MultipleChoiceQuestion question) {
		String correctUnit = question.getOptions().parallelStream().filter(e -> e.isCorrectOption()).map(e -> e.getId())
				.findAny().orElse("!" + answer.getValue());
	...
	public boolean answeredGatekeepers(String moduleId, int pageNum, String groupId) {
		return getGatekeepers(moduleId, pageNum).isEmpty() || entityManager
				.createQuery(
						...
						Boolean.class)
				.setParameter("groupId", groupId).setParameter("questions", getGatekeepers(moduleId, pageNum)
