...
public class AdminController {
	...
	public AstroAppUser updateUser(@PathVariable String email, @Valid @RequestBody AstroAppUser user,
			Principal principal) {
		// call service to persist user then return the user
		// check if user only has roles ADMIN and/or INSTRUCTOR else throw error

		// call mutator: user.email(email);

		// for (Role role : user.getRoles() {
		// if (AstroAppUser.Role.USER.equals(role) { throw some exception... }
		// }
		// checks to see if there are users in set with role user

		// if princimal.getName() is the same as user.getEmail(), make sure they
		// are not setting anything to expired
		// or removing the ADMIN role
